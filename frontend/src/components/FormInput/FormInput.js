  ...
  const handleSubmit = (event) => {
    ...
    if (input.query !== "") {
      axios.post("/generate", input)
        .then((response) => {
          ...
          if (response.status === false || response.data["error"] === true) {
            setOutput((oldState) => { 
              return (
                {...oldState, "status": response.data["status"], "error": true, "sqlToMongo": response.data["sql_to_mongo"] ?? null}
              )
            })
            setShowError(true);
          }
            ...
            setOutput((oldState) => { 
              return (
                {...oldState, "data": response.data["data"], "bestPlanId": response.data["best_plan_id"], "status": response.data["status"], "error": false, "sqlToMongo": response.data["sql_to_mongo"]}
              )
            })
            setShowSuccess(true);
          }
      ...
      .catch((error) => {
        ...
        setOutput((oldState) => { 
          return (
            {...oldState, "status": "Error generating output. Please check your query's formatting and/or validity.", "error": true, "sqlToMongo": null}
          )
        })
        setShowError(true);
      })
      ...
      setOutput((oldState) => { 
        return (
          {...oldState, "status": "Error generating output. Please input an SQL query.", "error": true, "sqlToMongo": null}
        )
      })
      setShowError(true);
    }
