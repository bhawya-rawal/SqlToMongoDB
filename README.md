...
This project **does not require Python/Flask/Poetry**.

1. Ensure you have **Node.js** and **npm** installed.
2. From this folder, install all workspace dependencies:
...
The backend connects to PostgreSQL using environment variables. Create `server/.env`:
...
You will have to connect to a Postgresql database populated with dummy data generated by [TPC-H](http://www.tpc.org/tpch/). Continue reading the installation instructions below if you have not generated the TPC-H dataset.

If you **haven't installed PostgreSQL/TPC-H yet**, you can still run the app and use the **SQL → MongoDB** conversion output. The optimizer/plan visualization features (EXPLAIN, selectivity, alternative plans) require PostgreSQL + a loaded TPC-H schema/data.

## Running the application

- To run both the server and frontend concurrently, run this from the root folder:
...
If you've already completed these two steps, feel free to ignore the setup instructions below and simply run `npm run dev` from the root folder in order to start both the frontend and the API server concurrently. Otherwise, do set up the following to ensure that the project runs correctly.

### Setting up PostgreSQL and the TPC-H dataset

1. Install PostgreSQL locally (Windows installer) and ensure `psql` is available in your terminal.
2. Create a database. This project defaults to a DB named `TPC-H` (note the hyphen):

```sql
CREATE DATABASE "TPC-H";
```

3. From the repository root, create the empty TPC-H tables:

```bash
psql -U postgres -d "TPC-H" -f db/dss.ddl
```

4. Generate TPC-H data using the TPC-H `dbgen` tool (it outputs `*.tbl` files).
5. Put the generated `*.tbl` files into `db/data/` (folder exists).
6. Convert the `.tbl` files into pipe-delimited `.csv` files (removes the trailing `|` on each line).

No Python required:

```bash
node db/clean-tpch.js db/data
```
...
8. Create constraints (primary keys, foreign keys, etc.):

```bash
psql -U postgres -d "TPC-H" -f db/dss.ri
```

9. Run `ANALYZE` (or use pgAdmin Maintenance → VACUUM/ANALYZE) so `pg_stats` is populated (needed for selectivity/histograms).
