...
app.post("/generate", async (req, res) => {
  ...
  const sqlToMongoResult = sqlToMongo(normalizedSql);

  try {
    const qepRoot = await explainQuery(normalizedSql);
    const { graph, explanation } = visualizeExplainQuery(qepRoot);

    const estimated_cost_per_row = calculateEstimatedCostPerRow(qepRoot);
    ...
    const selectivities = await getSelectivities(normalizedSql, predicates);
    ...
    if (originalPredicateSelectivityData.length > 0) {
      const newPlans = generatePlans(selectivities, normalizedSql);

      for (let i = 0; i < newPlans.length; i += 1) {
        const [newQuery, predicateSelectivityData] = newPlans[i];
        const altQep = await explainQuery(newQuery);
        const altViz = visualizeExplainQuery(altQep);
        ...
        allGeneratedPlans[i + 1] = {
          ...
          estimated_cost_per_row: calculateEstimatedCostPerRow(altQep),
        };
      }
    }

    const bestPlanId = getBestPlanId(allGeneratedPlans);
